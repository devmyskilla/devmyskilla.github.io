...
function patchFile(path,patcher){
  ...
  src=patcher(src,replaceOne);
  fs.writeFileSync(path,src);
}

patchFile('js/app.js',(src,replaceOne)=>{
  ...
  if(src.includes('async function initInlineEditorApp()'))throw new Error('js/app.js inline editor integration already present');
...
patchFile('js/platform-detail.js',(src,replaceOne)=>{
