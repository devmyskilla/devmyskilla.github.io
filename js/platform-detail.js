(function(root,factory){
  ...
  function listSection(title,items,cssClass,platformId,field){if(!items.length)return'';return`<section class="profile-panel ${cssClass}"><h2>${esc(title)}</h2><ul data-edit-kind="platform" data-edit-id="${esc(platformId)}" data-edit-field="${esc(field)}">${items.map(item=>`<li>${esc(item)}</li>`).join('')}</ul></section>`}
